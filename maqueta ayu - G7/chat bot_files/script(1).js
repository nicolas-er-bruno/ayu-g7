...
push("Profesora", "En que puedo ayudarte?", true); 
...
document.getElementById("btnChoice").onclick = function() {
	...
	window.setTimeout(function () {
        push("Alumno", "What about accessibility ?");        
    },0);
	...
	window.setTimeout(function () {
		...
		push("Profesora", "Excellent idea ! Do you want to talk about web or mobile accessibility ?", true, "poll1"); 
		rawPush('Alumno', '<button id="poll-web" class="btn btn-primary btn-poll">Web</button><button id="poll-mobile" class="btn btn-primary btn-poll">Mobile</button>');
    ...
    if (!silence) {
        if (from==="Alumno") {
            ...
            if (!document.hasFocus() || document.activeElement !== document.getElementById('chat-input')) {
                document.title = "Profesora says ... " + title;
            }
        }
    }
...
function playSound(filename){   
    document.getElementById("sound").innerHTML='<audio autoplay="autoplay"><source src="./chat bot_files/sound-chat.mp3" type="audio/mpeg" /><embed hidden="true" autostart="true" loop="false" src="./chat bot_files/sound-chat.mp3" /></audio>';
}
